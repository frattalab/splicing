...
read_majiq_out <- function(x) {
    ...
    res = res |> separate_rows(junctions_coords,probability_changing,probability_non_changing,ref_mean_psi,alt_mean_psi,sep = ';',convert = TRUE)
    
    res = res |> tidyr::separate(junctions_coords,into = c("start","end"),convert = TRUE)
