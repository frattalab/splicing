...
SAMPLE_NAMES_NOP = list(set(samples2['sample_name']))
...
rule allPSI:
    input:
        ...
        expand(os.path.join(MAJIQ_DIR,"psi_single","{sample}".replace(config['bam_suffix'],"") + ".psi.voila"), sample = SAMPLE_NAMES_NOP)
